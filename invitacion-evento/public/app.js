    ...
    try {
      ...
      if (!res.ok) {
        errorEl.textContent = res.status === 401
          ? 'Clave incorrecta.'
          : (data.error || `Error del servidor (${res.status}). Revisa los logs de Render.`);
        return;
      }
...
async function handleBulkAdd() {
  ...
  if (lines.length === 0) {
    errorEl.textContent = 'Pega al menos un nombre.';
    return;
  }
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      errorEl.textContent = data.error || 'No se pudo agregar la lista.';
      return;
    }
    textarea.value = '';
    errorEl.style.color = 'var(--teal)';
    errorEl.textContent = `${data.added} agregado${data.added === 1 ? '' : 's'}` + (data.skipped > 0 ? `, ${data.skipped} ya estaban en la lista` : '');
    refreshAdminList();
  } catch (e) {
    errorEl.textContent = 'No se pudo conectar con el servidor.';
  } finally {
