...
// Admin: agregar varios invitados a la vez (uno por línea)
app.post('/api/admin/guests/bulk', async (req, res) => {
  if (!requireAdmin(req, res)) return;
  try {
    const { usernames } = req.body;
    if (!Array.isArray(usernames) || usernames.length === 0) {
      return res.status(400).json({ error: 'Envía al menos un nombre.' });
    }
    ...
    console.error(err);
    res.status(500).json({ error: 'No se pudo agregar la lista de invitados.' });
  }
});
